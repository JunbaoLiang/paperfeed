...
- v1.2(2026-07-17):**新功能 — 站外已读论文(external read)**。用户在别处读到的论文可通过前端(Saved 页输入框)提交 arXiv 链接/ID,`POST /external-read`:论文不在库则现场从 arXiv 拉元数据入库;有向量则立即生成 `recall_source='external'` 的 impression(特征快照用同一实现当场计算,position=推理常数,model_version='external')+ `external_read` 事件;无向量则入 `external_reads` 队列,由 daily 管道在 embed 后材料化(保证快照相似度特征真实)。事件语义:画像权重 +3.0(同 save)、计入 interaction_count、图召回强正反馈种子、feed 排除已读、训练标签 2(豁免 visible 要求)、metrics_rollup 排除(非真实曝光,不计 CTR)。已知取舍:单正样本组对 lambdarank 无配对梯度,主要价值在重塑画像与用户上下文特征。**非 arXiv 期刊论文**:接受 DOI / 含 DOI 的出版商链接,经 Semantic Scholar 拉元数据,papers 表以 `doi:<doi>` 为键(arxiv_id 列兼作通用键);arXiv 的 DataCite DOI(10.48550/arXiv.*)归一为 arXiv 身份;此类论文只作画像/训练信号,不进推荐语料,enrich 跳过,出版商未授权摘要时仅按标题编码。
